import { Physics, useBox, useContactMaterial, usePlane, useSphere } from '@react-three/cannon'
...
const Spheres = ({ colors, number, size }) => {
  const [ref, { at }] = useSphere(() => ({
    args: [size],
    mass: 1,
    position: [Math.random() * 1.5, Math.random() * 2, Math.random() * 1.5],
  }))
...
const Boxes = ({ colors, number, size }) => {
  ...
  const [ref, { at }] = useBox((index) => {
		return {
			args,
			mass: 1,
			position: [0, (index * size) + (size/2), 0],
		}
  })
  // useFrame(() => {
  ...
  return (
    <instancedMesh receiveShadow castShadow ref={ref} args={[undefined, undefined, number]} onCreated={setPositions}>
